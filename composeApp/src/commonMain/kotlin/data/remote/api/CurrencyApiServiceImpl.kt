...
class CurrencyApiServiceImpl (private val preferencesRepository: PreferencesRepository) :CurrencyApiService {
  companion object {
    const val ENDPOINT = "https://api.currencyapi.com/v3/latest"
    const val API_KEY = Credentials.APIKEY_CREDENTIALS
  }

  init {
    println("CurrencyApiServiceImpl created with preferencesRepository: $preferencesRepository")
  }
    ...
    return try{
      ...
      if(response.status.value == 200) {
        ...
        val availableCurrencyCodes = apiResponse.data.keys
          .filter {
            CurrencyCode.entries.map { code->code.name }
              .toSet()
              .contains(it)
          }
